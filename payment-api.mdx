...
```mermaid
sequenceDiagram
    title Finverse Payment Links — online payments flow<br/>(user-initiated payment on payments page or invoice)

    box rgb(247,247,247) Merchant
        actor User
        participant MerchantUI as Merchant UI
        participant MerchantBE as Merchant backend/ERP
    end

    box rgb(235,245,255) Finverse
        participant FPUI as Finverse Payment UI
        participant FAPI as Finverse API
        participant Gateways as Payment Gateways<br/>(banks/cards)
    end

    Note over User, MerchantBE: Payment/invoice preflow<br/>(merchant-specific)

    Note over MerchantBE, FAPI: API authentication (api.prod.finverse.net)

    MerchantBE->>FAPI: [1] POST /auth/customer/token<br/>(client_id, client_secret)
    FAPI-->>MerchantBE: customer_token

    Note over MerchantBE, FAPI: Payment Link creation<br/>(mode = payment — runs payment + saves payment method if reusable)

    MerchantBE->>FAPI: [2] POST /payment_links<br/>(amount, external_user_id, unique_reference_id, metadata)
    ...
    Note over FPUI, Gateways: Finverse payment processing:<br/>user picks method & pays<br/>(async — some methods can take up to 48h to confirm)

    FPUI--)MerchantBE: HTTP 302 redirect to merchant redirect_url<br/>(when ui_mode = redirect)

    Note over MerchantBE, FAPI: Async handling — Payment Link status<br/>(webhooks + GET; show result when async)

    FAPI--)MerchantBE: [3a] Payment Link & Payment webhooks

    MerchantBE->>FAPI: [3b] GET /payment_links/{payment_link_id}<br/>(fallback + payment & payment-method details)

    FAPI-->>MerchantBE: Payment Link<br/>(session_status, payment, payment_method)

    activate MerchantBE
    Note right of MerchantBE: record Payment;<br/>record Payment Method for future pays
    deactivate MerchantBE

    Note over MerchantUI, MerchantBE: Show outcome from session_status

    alt Incomplete — session OPEN
    ...
    Note over MerchantBE, FAPI: Async handling: same pattern as above
